...
**Tech Stack:** Go 1.26, `bubbletea` v1.3.10, `bubbles` v1.0.0, `lipgloss` v1.1.0, `client-go` v0.37.0, `zap` v1.28.0, `go.uber.org/mock` v0.6.0 (via `tool` directive), `teatest` for view tests.
...
- **Dependency versions are pinned and verified to compose.** Do not upgrade them mid-plan: `bubbletea v1.3.10`, `bubbles v1.0.0`, `lipgloss v1.1.0`, `k8s.io/client-go v0.37.0`, `k8s.io/api v0.37.0`, `k8s.io/apimachinery v0.37.0`, `go.uber.org/zap v1.28.0`, `go.uber.org/mock v0.6.0`.
...
```go
	...
	program := tea.NewProgram(ui.New(logger, store), tea.WithAltScreen())
```
...
func (f fake) View() string                            { return f.id }
