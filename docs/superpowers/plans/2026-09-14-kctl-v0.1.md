...
### Help text: who owns which keys

No view may hardcode a key string. All help text goes through
`keys.HelpLine(bindings ...key.Binding)`, which renders `key:desc` from the bindings
themselves, so displayed keys can never drift from what the bindings actually do.

Ownership is split so the rule stays stable as views come and go:

- **The root model always renders `keys.HelpLine(m.keys.Quit)`.** Quit is global — it
  works from every view — so it is shown in exactly one place, on the root's own
  help row, in every task from Task 4 onward.
- **Views render only their own contextual keys and never include `m.keys.Quit`.**
  A view's help row lists what that view adds: `enter:select  esc:back`, `f:follow`,
  and so on.

This is why `internal/ui/app_internal_test.go`'s guard test works at every stage: the
root's help row always exists, so the test can rebind `Quit` and assert the rendered
output follows without having to move as views are added.

A key is advertised only in the task that binds it. Task 8's error state shows just
`esc` because `Retry` is not bound until Task 13, which adds it to both the keymap and
that help line together.
...
- Create: `internal/ui/keys/keys_test.go` (Step 5c)
- Modify: `internal/ui/keys/keys.go` — adds `HelpLine` (Step 5b)
- Modify: `internal/ui/app.go`
- Modify: `internal/ui/app_test.go`
- Modify: `internal/ui/app_internal_test.go` — retargets the Task 1 guard test (Step 5d)
...
- [ ] **Step 5b: Add the keymap-derived help-line helper**

Help text must never hardcode a key. Task 1 established this for the quit hint; from
here on, five views render multi-key help lines, so the rendering belongs in one place.

Append to `internal/ui/keys/keys.go`:
...
- [ ] **Step 5c: Write the failing helper test**

Create `internal/ui/keys/keys_test.go`:
...
Run: `go test ./internal/ui/keys/ -v`
Expected: FAIL (undefined: `keys.HelpLine`), then PASS once Step 5b is in place.

- [ ] **Step 5d: Retarget the Task 1 guard test onto the help line**

Task 1 left `internal/ui/app_internal_test.go` asserting the root view renders
`"press x to exit"`. This step's `View()` no longer renders that placeholder hint, so
that test must move rather than be deleted — deleting it would discard the
single-source-of-truth guarantee it exists to hold.

Replace its body so it still rebinds and still checks the rendered output, now
against the help line:
...
`contexts` is a mock `core.ContextManager` built the same way as in `app_test.go`.
Keep BOTH assertions: the negative one is what fails if a literal ever comes back.
