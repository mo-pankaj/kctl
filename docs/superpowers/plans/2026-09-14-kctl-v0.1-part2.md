...
- Create: `internal/ui/views/podlist/podlist_internal_test.go` (Step 2b)
...
- [ ] **Step 2b: Write the failing internal test for Enter's effect**

`SelectedMsg` is emitted for the ROOT model to consume, so it changes nothing on
podlist's own screen. That makes it invisible to a teatest assertion: checking this
view's output after pressing Enter would pass whether or not the message was ever
produced. Exercise the command directly instead.

Create `internal/ui/views/podlist/podlist_internal_test.go` (note: `package podlist`,
so it can reach the unexported `emitSelected`):
...
Run: `go test ./internal/ui/views/podlist/ -run TestEmitSelected -v`
Expected: FAIL (undefined), then PASS after Step 3.
...
- Modify: `internal/ui/views/podlist/podlist_internal_test.go` (Step 4b)
...
- [ ] **Step 4b: Write the failing test for Enter under an active filter**

Step 7 changes `emitSelected` to index `m.visible` rather than `m.pods`. That is the
single easiest bug in this task to introduce and the hardest to notice: with no filter
the two slices are identical, so an implementation that indexes the wrong one passes
every other test and only misbehaves once a filter narrows the list — at which point
Enter silently opens the wrong pod's logs.

Append to `internal/ui/views/podlist/podlist_internal_test.go`:

```go
func TestEmitSelectedIndexesTheFilteredListNotTheFullOne(t *testing.T) {
	pods := []core.Pod{
		{Namespace: "ns1", Name: "api-1"},
		{Namespace: "ns1", Name: "worker-2"},
	...
	// Visible is now [worker-2, worker-3]. Cursor 0 must select worker-2.
	// An implementation still indexing m.pods would return api-1.
	m.table.SetCursor(0)

	cmd := m.emitSelected()
	if cmd == nil {
		t.Fatal("emitSelected() returned a nil command with a filtered list")
	}

	selected, ok := cmd().(SelectedMsg)
	if !ok {
		t.Fatalf("emitSelected() produced %T, want SelectedMsg", cmd())
	}

	if selected.Pod.Name != "worker-2" {
		t.Fatalf("SelectedMsg.Pod.Name = %q, want %q — emitSelected is indexing the unfiltered slice", selected.Pod.Name, "worker-2")
	}
}
```

Run: `go test ./internal/ui/views/podlist/ -run TestEmitSelectedIndexes -v`
Expected: FAIL until Step 7 switches `emitSelected` to `m.visible`, then PASS.
...
// NOTE: the filter's clear-and-restore behaviour is asserted at the model level
// in podlist_internal_test.go, not here. teatest.WaitFor drains the output
// stream, so a test that waits three times (row present -> row gone -> row back)
// finds nothing on its third wait and fails for a reason unrelated to the code.
