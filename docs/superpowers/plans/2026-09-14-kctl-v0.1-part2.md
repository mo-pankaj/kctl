...
**All Global Constraints from Part 1 apply here unchanged.** In particular: `internal/core` imports no Kubernetes libraries, `internal/ui` imports no Kubernetes libraries, errors are checked on their own line and wrapped `-`-style, mocks are generated, and `teatest` assertions use `WaitFor` on `tm.Output()` rather than `FinalOutput`.
...
Add to `Update`, in the `tea.KeyMsg` case before routing:
...
```go
	...
	program := tea.NewProgram(ui.New(logger, store, source, sel), tea.WithAltScreen())
```
...
Route keys so a focused input wins. Place this at the top of the `tea.KeyMsg` case:
...
and in `Update`'s `tea.KeyMsg` case, before the `Quit` and `Back` checks:
...
In `internal/ui/views/podlist/podlist.go`, inside the `tea.KeyMsg` case, after the filter handling and before `Select`:
